...
def getRSS(ch, variable, model, dataset, fitRes, carddir, doplots, norm = -1, label = "nom"):
       ...
       #ratiohist seems to be for the ratio plot, but rss is passed on to the next steps
       for i in xrange(0, hist.GetN()): #index error because residuals.getY is 1 shorter than getN for some reason -> GetN()-1
              #print(i)
              #print(hist.GetN())
              #print(len(hist.GetX()))
...
def getCard(ch, useChi2 = False, doplots = True):
       ...
       for i in range(1,5): #change based on number of alt functions (1,5) for 4
              #if iPar==3:continue
