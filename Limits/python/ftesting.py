...
def getRSS(ch, variable, model, dataset, fitRes, carddir, doplots, norm = -1, label = "nom"):
       ...
       for i in xrange(0, hist.GetN()-1): #index error because residuals.getY is 1 shorter than getN for some reason
              #print(i)
              #print(hist.GetN())
              #print(len(hist.GetX()))
...
def getCard(ch, useChi2 = False, doplots = True):
       ...
       if doplots:
              ...
              #modelAlt[2].plotOn(xframeAlt, RooFit.Name("modelAlt3"), ROOT.RooFit.LineColor(ROOT.kRed)) #dont converge, take out for now
       ...
       for i in range(1,3): #change based on number of alt functions (1,5) for 4
